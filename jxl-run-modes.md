...
> 本文是仓库各启动链路（`virt` / `raspi3b` / `jxl` / `jxl-linux` /
> `jxl-linux-spl` / `jxl-xen` / `jxl-xen-atf` / `jxl-optee` /
> `jxl-xen-optee` / `linux`）的参考手册。覆盖：
>
> - 每个 `make build-X` / `./build.sh X` 产生什么文件、用在哪里
> - 每个 `make run-X` / `./start.sh X` 实际执行的 QEMU 参数、启动链、镜像来源
> - JXL 固定内存地址、flash 与 MMC 镜像布局
> - 启动链横向对比表
>
> 项目仓库：<https://github.com/PenNameLuXun/qemu-vp>
> 本文路径：<https://github.com/PenNameLuXun/qemu-vp/blob/main/jxl-run-modes.md>
>
> 入口说明在 [README.md](README.md)；U-Boot 启动链原理在
> [uboot-boot-chain.md](uboot-boot-chain.md)；Qt GUI 三种后端的差别在
...
- 这是当前仓库里较完整的启动链，覆盖 SPL / BL31 / U-Boot / Xen / Dom0 Linux
